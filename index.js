...
const { app, BrowserWindow, Menu, ipcMain, shell, dialog, session, MenuItem } =
electron;
var mainwindow;
...
const iconPath =
    process.platform !== "darwin" ? "src/icons/app.ico" : "src/icons/app.icns";
fs.mkdirSync(path.join(__dirname, "src/icons"), { recursive: true });
// const icongen = require("icon-gen"); //Install npm icon-gen
const console = require("console");
// icongen(
//         path.join(__dirname, "src/titlebar/icon.png"),
//         path.join(__dirname, "src/icons"), { report: true }
//     )
//     .then((results) => {
//         try {
//             fs.copyFile(
//                 path.join(__dirname, "src/icons/app.ico"),
...
//             );
//             fs.copyFile(
//                 path.join(__dirname, "src/icons/app.icns"),
...
app.on("ready", () => {
    ...
    mainwindow.webContents.on("did-start-loading", (e, i) => {
        mainwindow.webContents.executeJavaScript(
            fs.readFileSync(path.join(__dirname, "src/titlebar/titlebar.js"))
        );
    });
    ...
    mainwindow.on("close", (e) => {
        if (!allowedToExit) e.preventDefault();
    });
        ...
        (event, item, webContents) => {
            if (allowedToExit) {
                if (item.getURL().startsWith("blob:")) {
                    item.setSavePath(
                        generateUniqueName(
                            path.join(getDownloadLocation(), item.getFilename())
                        )
                    );
                    //downloadblob(null, item.getURL(), item.getFilename());
                } else {
                    item.setSavePath(
                        generateUniqueName(
                            path.join(getDownloadLocation(), item.getFilename())
                    ...
                    session.defaultSession.cookies
                        ...
                        .then((cookies) => {
                            ...
                            // cookiesMain = cookiesMain.substring(0, cookiesMain.length - 2);
                        })
                        .catch((error) => {
                            console.log(error);
                        });
        ...
        new MenuItem({
            ...
            click: () => {
                if (allowedToExit) {
                    ...
                    savedFileName = decodeURIComponent(
                        savedUrl.split("/").pop().split("#")[0].split("?")[0]
                    );
...
const getDownloadLocation = function() {
    if (saveLocation == null) return app.getPath("downloads");
...
const generateUniqueName = function(filename, tries = 0) {
    ...
    return generateUniqueName(
        name + "(" + (tries + 1) + ")." + nameAndExt[1],
        tries + 1
    );
};
const getDownloadSizeAndCheckIfSupportsRange = function(
    url,
    successfullCallback,
    failedCallback
) {
    ...
    req.on("error", function(e) {
        if (e.message != "aborted") failedCallback(e.message);
    });
};
...
ipcMain.on("download-start", (event, url, filename, threads) => {
    ...
    if (url.startsWith("http://") || url.startsWith("https://")) {
        ...
        event.reply("rangenotsupport", null);
        downloadblob(event, url, filename);
    }
});
...
ipcMain.on("openLocationSelector", (event, args) => {
    try {
        saveLocation = dialog.showOpenDialogSync(mainwindow, {
...
ipcMain.on("quit", (event) => {
    if (allowedToExit) app.quit();
    else
        dialog.showMessageBox(mainwindow, {
            title: "Downloader",
            message: "Please wait for the download to finish",
        });
    //mainwindow.minimize();
});
